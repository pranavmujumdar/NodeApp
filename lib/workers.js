...
workers.log = function(originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck){
  ...
  _logs.append(logFileName, logString, function(err){
    if(!err){
      ...
      console.log('Error writing the log to the file');
    }
  })
          ...
          if(!err){
            ...
            _logs.truncate(logId, function(err){
              if(!err){
                ...
                console.log('error truncating log files', err);
              }
            })
          }
          else{
            console.log('Error compressing the file logs', err);
          }
        })
      })
    }
    else {
      console.log('Couldn\'t find any logs to compress')
    }
  })
}
