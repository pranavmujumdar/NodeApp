    ...
    if (phone) {
        ...
        handlers._tokens.isTokenValid(token, phone, function(tokenIsValid){
            if(tokenIsValid){
                ...
                callback(403, {'Error': 'Token Invalid or missing'})
            }
        });
    } else {
        callback(400, {
    ...
    if(phone && password){
        _data.read('users', phone, function(err,userData){
            if(!err && userData){
                ...
                callback(400, {'Error':'User don\'t exist mf'})
            }
        });
    }
    else{
    ...
    if(id && extend){
        ...
        _data.read('tokens', id, function(err, tokenData){
            if(!err && tokenData){
                ...
                if(tokenData.expires > Date.now()){
                    ...
                    _data.update('tokens', id,tokenData, function(err){
                        if(!err){
                            ...
                            callback(500, {'Error' : 'Error updating the token\'s Expiration' })
                        }
                    })
                }
                else{
                ...
                callback(400, {'error':'error finding the token'});
            }
        })
    }
    else{
        callback(400, {'Error': 'Missing required Fields, or '})
    }
};
...
handlers._checks.post = function(data, callback){
    ...
    if(protocol && url && method && successCodes && timeOutSeconds){
            ...
            if(!err && tokenData){
                ...
                _data.read('users', userPhone, function(err, userData){
                    if(!err && userData){
                                ...
                                if(!err){
                                    ...
                                    _data.update('users', userPhone, userData, function(err){
                                        if(!err){
                                            ...
                                            callback(500, {'Error': 'Could not associate the check with the user'})
                                        }
                                    })
                                }
                                else{
                        ...
                        callback(400, {'Error': 'User has already maximum checks ('+config.maxChecks+')'})
                    }
                })
            }
            else{
        ...
        callback(400,{'Error':'Invalid inputs for checks'});
    }
}
                        ...
                        if(tokenIsValid){
                            _data.read('checks', id, function (err, checkData) {
                                if(!err && checkData){
                                    ...
                                    callback(500, {'Error': 'Error reading the check data'})
                                }
                            })
                        }
                        else
                        ...
                        _data.delete('checks', id, function(err){
                            if(!err){
                                ...
                                _data.read('users', checkData.userPhone, function(err, userData){
                                    ...
                                    {
                                        callback(500, {'Error':'Could Not find the user to delete their Checks'});
                                    }
                                })
                            }
                            else
