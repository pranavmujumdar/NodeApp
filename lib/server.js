...
server.commonServer = (req, res) => {
  ...
  req.on('end', () => {
    ...
    chosenHandler(data, function (statusCode, payload) {
      ...
      // log the response, if the response is 200 print green, else red
      if(statusCode == 200){
      ...
      //debug('Returning this string ', statusCode, payloadString);
    });
...
server.init = function(){
  ...
  server.httpServer.listen(config.httpPort, () => {
    console.log('\x1b[32m%s\x1b[0m', "Server up on " + config.httpPort + " on the " + config.envName + " environment");
  });

  // Start the HTTPS Server  
  server.httpsServer.listen(config.httpsPort, () => {
    console.log('\x1b[32m%s\x1b[0m', "Server up on " + config.httpsPort + " on the " + config.envName + " environment");
  })
}
